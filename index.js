...
function cocukSarkisi(x) {
  return `${5} küçük maymun yatakta zıplamış, biri düşüp başını çarpmış, Anne doktoru aramış, Doktor çok kızmış: Bir daha yatakta zıplamak yok!` 
...
function notHesapla(x) {
  if (x>=90) {return 'A aldın'}
  else if (x>=80){return 'B aldın'}
  else if (x>=70){return 'C aldın'}
  else if (x>=60){return 'D aldın'}
  else {return 'F aldın'}
}
...
function sesliHarfSayaci(araba) {
